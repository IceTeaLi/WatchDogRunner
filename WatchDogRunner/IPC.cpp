...
const FMessageName MessageBase::WindowsMessageNames[1100] =
{
	{ 0, TEXT("WM_NULL") },
	{ 1, TEXT("WM_CREATE") },
	{ 2, TEXT("WM_DESTROY") },
...
const Message& ipc::message::MessageBase::TranslateCopydataMsg(const MSG& copy_data)
{
	Message message = {};
	COPYDATASTRUCT temp = *(PCOPYDATASTRUCT)copy_data.lParam;
	int type = (int)copy_data.wParam;
	if (type == WH_GETMESSAGE)
	{
		MSG msg = *(PMSG)temp.lpData;
		message << msg;
	}
	else if (type == WH_CALLWNDPROC)
	{
		CWPSTRUCT cwp = *(PCWPSTRUCT)temp.lpData;
		message << cwp;
	}
...
MsgIPCWnd::MsgIPCWnd(QWidget* parent) :
	QWidget(parent)
{
	cache = new Cache;
